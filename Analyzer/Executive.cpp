...
int main(int argc, char* argv[])	// test executive for CodeAnalyzer
{
  ...
  try {
	...
	std::cout << std::endl;
	title("REQUIREMENT #3 - Provided the following relevant C++ packages: TypeTable, DependencyAnalysis, NoSQLDb, StrongComponent, XmlPersist, CodeAnalyzer(Executive)");
   ...
   /* Rslt::write(out.str());    Rslt::write("\n");*/    exec.stopLogger();
    ...
    TypeAnal ta;
	title("\n  REQUIREMENT #4 - Identifies all of the types defined in a specified file.");
	ta.doTypeAnal();
	title("\n  REQUIREMENT #5 -  Identifies all of the dependencies between files in a specified file collection.");
	title("DEPENDENCY TABLE");
	ta.doDepAnal();
	title("\n  REQUIREMENT #6 - Finding strong components in the dependency graph");
	ta.GraphStrongComponent();
	title("REQUIREMENT #7 - Writing the analysis results, in XML format, to a specified file.");
	title("Default location: ../Analyzer/XML_DependencyAnalysis.txt");
	ta.XmlFileName(argv[argc - 1]);
	title("REQUIREMENT #8 - Processing the command line: ../FilesToTest *.h *.cpp /m");
	title("REQUIREMENT #9 - Includes a test executive that demonstrates all the requirements of this project");
  }
